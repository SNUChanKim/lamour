...
class HumanoidNormalEnv(MuJocoPyEnv, utils.EzPickle):
    """Modified DMC Humanoid environment to match OOD format.

    This environment follows the format of HumanoidOODEnv but maintains the reward
    structure of the original HumanoidNormalEnv.
    """
    ...
    def disable(self, *flags):
        old_disable_flags = self.sim.model.opt.disableflags
        new_disable_flags = old_disable_flags
        for flag in flags:
            if isinstance(flag, str):
                flag = flag.upper()
                field_name = 'DISABLE_' + flag if not flag.startswith('DISABLE_') else flag
                new_disable_flags = new_disable_flags | getattr(self.sim.model.opt, field_name)
            else:
                new_disable_flags = new_disable_flags | flag
        self.sim.model.opt.disableflags = new_disable_flags
        try:
            yield
        finally:
            self.sim.model.opt.disableflags = old_disable_flags