...
class AntOODEnv(MuJocoPyEnv, utils.EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def is_healthy(self):
        ...
        roll, pitch, yaw = euler_from_quaternion(observation[1], observation[2], observation[3], observation[4])
        min_z, max_z = self._healthy_z_range
        is_healthy = (np.isfinite(state).all() and min_z <= state[2] <= max_z) and not self._check_flip(roll, pitch)
        return is_healthy
    ...
    def reset_model(self):
        ...
        self.return_to_in_distribution = False
        qx, qy, qz, qw = euler_to_quaternion(math.pi, 0, 0)
        qpos = self.init_qpos
        qpos[3] = qw
        qpos[4] = qx
        qpos[5] = qy
