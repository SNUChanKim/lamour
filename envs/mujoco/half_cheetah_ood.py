...
class HalfCheetahOODEnv(MuJocoPyEnv, utils.EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    @property
    def is_healthy(self):
        is_healthy = not (self.sim.data.qpos[2]>math.pi/2.0 or self.sim.data.qpos[2]<-math.pi/2.0)
        return is_healthy
    
    @property
    def terminated(self):
        terminated = False
        if not self.is_healthy and self.return_to_in_distribution:
            terminated = True
        return terminated
