...
from envs.mani_skill2 import ASSET_DIR, logger
from envs.mani_skill2.agents.base_agent import AgentConfig, BaseAgent
from envs.mani_skill2.sensors.camera import (
    Camera,
    CameraConfig,
    parse_camera_cfgs,
    update_camera_cfgs_from_dict,
)
from envs.mani_skill2.sensors.depth_camera import StereoDepthCamera, StereoDepthCameraConfig
from envs.mani_skill2.utils.common import convert_observation_to_space, flatten_state_dict
from envs.mani_skill2.utils.sapien_utils import (
    get_actor_state,
    get_articulation_state,
    set_actor_state,
    set_articulation_state,
)
from envs.mani_skill2.utils.trimesh_utils import (
    get_actor_meshes,
    get_articulation_meshes,
    merge_meshes,
)
from envs.mani_skill2.utils.visualization.misc import observations_to_images, tile_images
...
class BaseEnv(gym.Env):
    ...
    def set_episode_rng(self, seed):
        ...
        if seed is None:
            ...
            self._episode_rng, _ = seeding.np_random(seed)
            return
            
        if self._episode_seed is None:
            self._episode_rng, self._episode_seed = seeding.np_random(seed)
