...
from envs.mani_skill2.agents.robots.mobile_panda import MobilePandaDualArm
from envs.mani_skill2.utils.common import np_random
from envs.mani_skill2.utils.geometry import transform_points
from envs.mani_skill2.utils.sapien_utils import get_entity_by_name, vectorize_pose
from envs.mani_skill2.utils.trimesh_utils import get_actor_visual_mesh

from envs.mani_skill2.envs.base_env import MS1BaseEnv


class PushChairOODEnv(MS1BaseEnv):
    DEFAULT_MODEL_JSON = (
    ...
    def evaluate(self, **kwargs):
        ...
        flags = dict(
            ...
            chair_standing=chair_tilt < 0.2 * np.pi,
        )
        return dict(
    ...
    def compute_dense_reward(self, action: np.ndarray, info: dict, **kwargs):
        ...
        if dist_chair_to_target < 0.15 and chair_tilt < 0.2 * np.pi:
            stage_reward += 100
    ...
    def get_done(self, info: dict, **kwargs):
        if self.return_to_in_distribution:
            chair_standing = info["chair_tilt"] < 0.2 * np.pi
            done = bool(info["success"] or not chair_standing)
        else:
            done = False
        return done
