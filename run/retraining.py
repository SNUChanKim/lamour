...
from run.training import set_training, set_logging, evaluate, save_checkpoint
...
def step_vectorized(args, env, agent, memory, writer, states, total_steps, infos=None):
    ...
    for i in range(batch_size):
        ...
        # Handle reward modifications based on policy
        if args.use_own_crit:
            if 'sero' in args.policy and deg_uncertainties[i].mean() > args.uncertainty_th:
                modified_rewards[i] = 0
            elif 'lamour' in args.policy and not is_recovereds[i]:
                modified_rewards[i] = 0
            elif infos is not None and hasattr(infos[i], 'get'):
                modified_rewards[i] = infos[i].get('original_reward', modified_rewards[i])
        
        if args.use_env_reward and infos is not None and hasattr(infos[i], 'get'):
            modified_rewards[i] = infos[i].get('original_reward', modified_rewards[i])
...
def step(args, env, agent, memory, writer, state, total_steps, info=None):
    ...
    _reward = reward
    if args.use_own_crit:
        if 'sero' in args.policy and deg_uncertainty.mean() > args.uncertainty_th:
            _reward = 0
        elif 'lamour' in args.policy and not is_recovered:
            _reward = 0
        else:
...
def retrain():
    ...
    # Update policy name based on configurations
    if args.policy == 'sero':
        if args.use_own_crit:
